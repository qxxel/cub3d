...
/*   Updated: 2025/01/21 16:51:36 by mreynaud         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	put_txr_wall(t_game *game, float percent_y, float x_ray, float y_ray, float angle)
{
	...
	// 	(y_ray >= 1 - 1 / 64.0 && angle < PI && angle > 0)) && \
	// 	((x_ray <= 1 / 64.0 && (angle < PI / 2 || angle > 3 * PI / 2)) || \
	// 	(x_ray >= 1 - 1 / 64.0 && angle > PI / 2 && angle < 3 * PI / 2)))
	// 	return (255);
	if (y_ray >= 1 - 1 / 64.0 && angle < PI && angle > 0)
		return(put_img_wall(&txr->north, percent_y, x_ray)); // return (0xFFFF00); // yellow
	else if (y_ray <= 1 / 64.0 && angle > PI && angle < 2 * PI)
...
static void	display_wall(t_game *game, float x_ray, float y_ray, int *i, float angle)
{
	...
	{
		put_pixel(&game->img_data, put_txr_wall(game , (j - start) / wall_height, x_ray, y_ray, angle), *i, j);
		j++;
	}
	while (j < HEIGHT)
